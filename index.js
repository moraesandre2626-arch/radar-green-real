...
function checaJogo(jogo, raio){
  ...
  if(scorePressao < FILTROS.min_pressao_score) return {aprovado:false, motivo:`score baixo ${scorePressao.toFixed(1)}`};
  if(posseTime < FILTROS.posse_minima_pressao) return {aprovado:false, motivo:`posse pressao ${posseTime}% < ${FILTROS.posse_minima_pressao}%`};
...
async function enviarTelegram(jogo, analise, raio){
  ...
  try{
    ...
    totalEnviados++; console.log(`📨 Enviado: ${jogo.nome_casa} x ${jogo.nome_fora} | ${analise.zonaPressao} | ${analise.possePct}%`); return true;
    ...
    try{
      ...
      for(const ev of data.events){
        try{
          ...
          const analise=checaJogo(jogo, raioTemp); if(!analise.aprovado){ if(analise.motivo?.includes('posse')) console.log(`🚫 Posse <60%: ${jogo.nome_casa} ${jogo.gols_casa}x${jogo.gols_fora} ${jogo.nome_fora} - ${analise.motivo}`); continue; }
...
async function executarRadar(){
  ...
  try{
    const res=await buscarJogosESPN(); if(!res.length){ console.log('Nenhum aprovado (precisa 60% posse).'); return; }
...
const server = http.createServer((req,res)=>{ res.writeHead(200,{'Content-Type':'application/json'}); res.end(JSON.stringify({status:'online',robo:'V34.6 60% POSSE PRESSAO',ultimoScan,aprovados:totalAprovados,enviados:totalEnviados, filtros:FILTROS},null,2)); });
server.listen(PORT, ()=>{ console.log(`🚀 V34.6 ONLINE porta ${PORT}`); executarRadar(); setInterval(executarRadar,60000); });
